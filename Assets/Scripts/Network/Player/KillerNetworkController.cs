...
public class KillerNetworkController : NetworkBehaviour
{
    ...
    public void Attack()
    {
        ...
        if (attackColliders != null)
        {
            foreach(var ac in attackColliders)
            {
                ...
                {
                    HumanNetworkController hitHuman;
                    bool isHumanControllerAvailable = ac.transform.gameObject.TryGetComponent<HumanNetworkController>(out hitHuman);
